...
describes what users get. Pick a format with `-o`, or let cataas choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cataas <command> -o table   # aligned columns for reading
cataas <command> -o jsonl   # one JSON object per line, for piping
cataas <command> -o json    # a single JSON array
cataas <command> -o csv     # spreadsheet friendly
cataas <command> -o tsv     # tab-separated
cataas <command> -o url     # just the URL column
cataas <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cataas <command> --fields id,title,url
```
...
```bash
cataas <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cataas <command>            # a table, because this is a terminal
cataas <command> | wc -l    # JSONL, because this is a pipe
```
